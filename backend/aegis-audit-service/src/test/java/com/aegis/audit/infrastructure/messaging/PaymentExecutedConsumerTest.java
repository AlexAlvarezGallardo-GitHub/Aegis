...
@DisplayName("PaymentExecutedConsumer - Kafka Consumer")
class PaymentExecutedConsumerTest {
    ...
    private PaymentExecutedConsumer consumer;

    @BeforeEach
    void setUp() {
        consumer = new PaymentExecutedConsumer(paymentAuditRecordService, processedEventRepository);
    }
    ...
    void shouldMapAndPersistPaymentAuditRecord() {
        ...
        // Act
        consumer.consume(event, "payment.executed", 0, 7L);
        ...
        assertEquals("EXECUTED", saved.eventType());
        ...
        verify(processedEventRepository).insertIfAbsent(eq(eventId), eq("payment.executed"),
                eq(0), eq(7L), any());
    }
    ...
    void shouldHandleNullPayee() {
        // Arrange
        PaymentExecutedEvent event = new PaymentExecutedEvent(
                UUID.randomUUID(), "PAYMENT_EXECUTED", "1.0",
        ...
        // Act
        consumer.consume(event, "payment.executed", 0, 8L);
    ...
    void shouldSkipDuplicateEvents() {
        ...
        // Act
        consumer.consume(event, "payment.executed", 2, 33L);
