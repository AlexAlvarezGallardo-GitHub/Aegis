...
/**
 * Kafka consumer that listens for {@code payment.executed} events.
 * <p>
 ...
 */
@Component
public class PaymentExecutedConsumer {

    private static final Logger log = LoggerFactory.getLogger(PaymentExecutedConsumer.class);

    private final PaymentAuditRecordService paymentAuditRecordService;
    private final ProcessedEventJpaRepository processedEventRepository;

    public PaymentExecutedConsumer(PaymentAuditRecordService paymentAuditRecordService,
    ...
    /**
     * Consumes a {@link PaymentExecutedEvent} from Kafka, maps it to a
     * {@link PaymentAuditRecord}, and persists it to the database.
     *
     * @param event     the deserialized payment executed event
     * @param topic     the source topic
     * @param partition the source partition
     * @param offset    the source offset
     */
    @KafkaListener(
            topics = "${aegis.kafka.topics.payment-executed}",
            groupId = "${spring.kafka.consumer.group-id}",
            containerFactory = "paymentExecutedListenerContainerFactory"
    )
    @Transactional
    public void consume(PaymentExecutedEvent event,
                        @Header(KafkaHeaders.RECEIVED_TOPIC) String topic,
                        @Header(KafkaHeaders.RECEIVED_PARTITION) int partition,
                        @Header(KafkaHeaders.OFFSET) long offset) {
        log.info("Received PaymentExecutedEvent: eventId={}, paymentId={}, amount={} {}",
