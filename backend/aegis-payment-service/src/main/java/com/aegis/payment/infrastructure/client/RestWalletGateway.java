...
public class RestWalletGateway implements WalletGateway {
    ...
    @Override
    public BigDecimal creditRefund(UUID refundId, UUID walletId,
                                   BigDecimal amount, String currency) {
        Map<String, Object> body = new LinkedHashMap<>();
        ...
        try {
            JsonNode response = restClient.post()
                    .uri("/api/v1/wallets/{walletId}/credits/refund", walletId)
                    .body(body)
                    .retrieve()
                    .body(JsonNode.class);
            if (response == null || !response.has("newBalance")) {
                throw new SettlementFailedException("Wallet service did not return new balance after refund credit");
            }
            return response.get("newBalance").decimalValue();
        } catch (RestClientResponseException ex) {
            log.warn("Wallet service rejected refund credit for refund {}: HTTP {}",
                    refundId, ex.getStatusCode());
            String code = extractErrorCode(ex);
            throw new SettlementFailedException(
                    code, "Wallet service rejected refund credit (HTTP " + ex.getStatusCode() + ")", ex);
        } catch (ResourceAccessException ex) {
            log.warn("Wallet service unreachable during refund credit for refund {}: {}",
            ...
            log.error("Unexpected error crediting refund {}", refundId, ex);
            throw new SettlementFailedException(
                    "Unexpected wallet service error: " + ex.getMessage(), ex);
        }
    }
